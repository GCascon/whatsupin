var app = {
  ...
  pintaCoordenadasEnMapa: function(position){
    ...
    app.pintaMarcador([position.coords.latitude, position.coords.longitude], '¡Estoy aquí! <a href="geo:38.897096,-77.036545">open map</a>'+position.coords.speed, miMapa);

    app.pintaMarcador([40.4170, -3.7036], 'Puerta del Sol', miMapa);
    app.pintaMarcador([40.4184, -3.7131], 'Palacio Real', miMapa);
    app.pintaMarcador([40.4168, -3.6837], 'Parque del Retiro', miMapa);

    miMapa.on('click', function(evento){
      var texto = 'Marcador en l(' + evento.latlng.lat.toFixed(4) + ') y L(' + evento.latlng.lng.toFixed(4) + ')';
